...
void GameApp::UpdateScene(float dt)
{
    if (ImGui::Begin("Texture Mapping"))
    {
        ...
        if (ImGui::Combo("Mode", &curr_mode_item, mode_strs, ARRAYSIZE(mode_strs)))
        {
            ...
            {
                //alt
                m_CurrMode = ShowMode::MyBox;
                m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
                auto meshData = Geometry::CreateBox(2.0f);
                ResetMesh(meshData);
                m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
                m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
                //使用多资源的格式
                ID3D11ShaderResourceView** m_pTextureViews[] = { m_pMyBox.GetAddressOf(),m_pFlareAlpha.GetAddressOf() };
                m_pd3dImmediateContext->PSSetShaderResources(0, 2, *m_pTextureViews);
            }
        }
    }
    ImGui::End();
    ...
    if (m_CurrMode == ShowMode::WoodCrate)
    {
        ...
        XMMATRIX R = XMMatrixTranslation(-0.5f, -0.5f, 0.0f)*XMMatrixRotationZ(theta)* XMMatrixTranslation(0.5f, 0.5f, 0.0f);
        m_VSConstantBuffer.world = XMMatrixTranspose(W);
        m_VSConstantBuffer.worldR = XMMatrixTranspose(R);
    ...
    else if (m_CurrMode == ShowMode::MyBox)
    {
        static float phi = 0.0f, theta = 0.0f;
        phi += 0.00005f, theta += 0.000030f;
        XMMATRIX W = XMMatrixRotationX(phi) * XMMatrixRotationY(theta);
        XMMATRIX R = XMMatrixTranslation(-0.5f, -0.5f, 0.0f)*XMMatrixRotationZ(theta)* XMMatrixTranslation(0.5f, 0.5f, 0.0f);
        m_VSConstantBuffer.world = XMMatrixTranspose(W);
        m_VSConstantBuffer.worldR = XMMatrixTranspose(R);
...
bool GameApp::InitEffect()
{
    ...
    // 创建顶点着色器(2D)
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Basic_2D_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    // 创建像素着色器(2D)
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Basic_2D_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    HR(m_pd3dDevice->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pPixelShader2D.GetAddressOf()));

    // 创建顶点着色器(3D)
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Basic_3D_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    // 创建像素着色器(3D)
    //HR(CreateShaderFromFile(nullptr, L"HLSL\\Basic_3D_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    //HR(m_pd3dDevice->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pPixelShader3D.GetAddressOf()));

    //alt
    HR(CreateShaderFromFile(nullptr, L"HLSL\\Basic_3D_PS_alt.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    HR(m_pd3dDevice->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pPixelShader3D.GetAddressOf()));
    return true;
}
...
bool GameApp::InitResource()
{
    ...
    //alt
    HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flare.dds", nullptr, m_pMyBox.GetAddressOf()));
    ...
    m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate.GetAddressOf());
    //m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pMyBox.GetAddressOf());
    m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
